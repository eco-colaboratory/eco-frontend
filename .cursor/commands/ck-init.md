---
description: Bootstrap .cursor/ configuration for another project — interactive wizard covering bundles, skills, hooks, CLAUDE.md, and .ck.json. Also: --show to print current config, --reset to wipe it.
---
...
**Q: Conflict** — only if `<target>/.cursor/` already exists:
"`.cursor/` already exists in that project. How should we proceed?"
...
For each selected skill, copy `skills/<name>/` recursively to `<target>/.cursor/skills/<name>/`, excluding any `node_modules/` subdirectory. In Merge mode, skip skills directories that already exist at the destination.
...
mkdir -p <target>/.cursor/coding-levels
mkdir -p <target>/.cursor/rules
mkdir -p <target>/.cursor/contexts
```

**5b. Copy core files (always)**

From `the Cursor workspace root (the folder that contains this `.cursor` directory for this kit)/.cursor/` → `<target>/.cursor/`:
...
For each skill selected in Step 2, copy `the Cursor workspace root (the folder that contains this `.cursor` directory for this kit)/.cursor/skills/<name>/` → `<target>/.cursor/skills/<name>/` recursively, skipping any `node_modules/` subtrees. In Merge mode, skip skill directories that already exist at the destination.
...
```json
"ignorePatterns": ["**/bin/**","**/obj/**","**/.vs/**","**/.git/**","**/Migrations/**","**/TestResults/**","**/coverage/**","**/.cursor/session-data/**","**/*.suo","**/*.user","**/*.lock.json","**/node_modules/**"],
"env": { "CLAUDE_CODE_DISABLE_1M_CONTEXT": "true" }
```

Write assembled JSON (no comments) to `<target>/.cursor/settings.json`.
...
Path-scoped design rules live in `.cursor/rules/`:
